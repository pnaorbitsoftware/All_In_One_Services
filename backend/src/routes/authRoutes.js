...
router.patch("/profile/complete", requireAuth, async (req, res) => {
  try {
    ...
    const formattedPhone = cleanPhone.length === 12 && cleanPhone.startsWith("91") ? cleanPhone.slice(2) : cleanPhone;
    if (!/^\d{10}$/.test(formattedPhone)) {
      return res.status(400).json({ message: "Enter a valid 10-digit mobile number before completing the client profile." });
    }
