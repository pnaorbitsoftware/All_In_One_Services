...
const loadCatalog = () => {
  if (!catalogLoadPromise) {
    catalogLoadPromise = Promise.all([
      ...
      Provider.find({ isActive: true, approvalStatus: "approved" })
        .select("name businessName category customCategory location preferredWorkLocation rating reviews responseTime price description about features profileImage approvalStatus isActive")
