...
export default function ProviderProfileSheet({ visible, provider, submitting, onClose, onSubmit }) {
  ...
  const pickProviderImage = async () => {
    try {
      ...
      if (!permission.granted) {
        showConfirm("Photo permission", "Allow photo access to add your provider profile picture.");
        return;
      }
      ...
      showConfirm("Provider photo", "Could not select this image. Please try another photo.");
    }
  };
  ...
  const pickAadhaarCard = async () => {
    try {
      const permission = await ImagePicker.requestMediaLibraryPermissionsAsync();
      if (!permission.granted) {
        showConfirm("Aadhaar upload", "Allow photo access to upload your Aadhaar card.");
        return;
      }
