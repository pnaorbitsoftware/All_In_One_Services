...
export async function apiRequest(path, options = {}) {
  ...
  // Deduplicate concurrent identical GET requests (e.g. two screens/effects
  // requesting the same dashboard at the same time on app start). Only GETs
  // are safe to share since they have no side effects; the shared promise is
  // removed as soon as it settles, so no stale caching occurs.
  if (method === "GET") {
...
export const bookingApi = {
  ...
  cancel: (token, bookingId, reason = "") =>
    apiRequest(`/bookings/${bookingId}/cancel`, { method: "PATCH", token, body: { reason } }),
...
export const providerApi = {
  ...
  bookingTracking: (token, bookingId) => apiRequest(`/providers/bookings/${bookingId}/tracking`, { token }),
  ...
  rejectBooking: (token, bookingId, reason) =>
    apiRequest(`/providers/bookings/${bookingId}/reject`, {
      method: "PATCH",
      token,
      body: { reason },
    }),
