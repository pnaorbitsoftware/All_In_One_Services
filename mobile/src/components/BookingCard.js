    ...
    <View style={[styles.card, { backgroundColor: theme.surface, borderColor: theme.border }]}>
      <View style={styles.header}>
        <View style={styles.titleWrap}>
          ...
          {booking.bookingId ? (
            <Text style={[styles.bookingIdText, { color: theme.textMuted }]} numberOfLines={1}>
              #{booking.bookingId}
            </Text>
          ) : null}
      ...
      <View style={styles.info}>
        ...
        {String(booking.status).toLowerCase() === "completed" && booking.completedAt ? (
          <InfoLine icon="calendar-check-outline">
            Completed on {formatBookingDate(booking.completedAt)}
          </InfoLine>
        ) : null}
        {Number(booking.clientRating) >= 1 ? (
          <InfoLine icon="star-outline">
            {booking.clientRating}/5{booking.clientReview ? ` - "${booking.clientReview}"` : ""}
          </InfoLine>
        ) : null}
      </View>
      {canTrack ? (
