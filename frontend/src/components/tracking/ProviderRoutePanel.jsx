import { CheckCircle2, Clock, LocateFixed, MapPin, MessageCircle, Navigation, Phone, Play, Route, UserRound } from "lucide-react";
...
export default function ProviderRoutePanel({ booking, updateProviderBookingStatus, setStatusMessage, apiUrl }) {
  ...
  const canStartJob = !isClosed && ["arrived", "job_started", "completed"].includes(normalizedStatus);
  const canFinishJob = !isClosed && ["job_started", "completed"].includes(normalizedStatus);
  ...
  return (
    <section className="servicetrack-panel provider">
      ...
      <div className="servicetrack-map-area">
        <GoogleRouteMap
          ...
          clientLabel="Client"
        />
        <div className="servicetrack-map-overlay-top">
          <div className="servicetrack-map-label">
            <span className="servicetrack-map-dot orange" />
            Navigate to client
          </div>
          <div className="servicetrack-map-badge blue">{routeSummary?.distance || "Route pending"}</div>
        </div>
      </div>

      <div className="servicetrack-card-body">
        {(error || !hasCoordinates(clientLocation)) && (
          <div className="servicetrack-map-error">
            {error || "Client destination needs GPS or a geocodable address before routing can start."}
          </div>
        )}
        ...
        <button
          type="button"
          className="servicetrack-maps-button"
          onClick={openClientDirections}
          disabled={locatingProvider || (!hasCoordinates(clientLocation) && !clientAddress.trim())}
        >
          <Navigation size={18} />
          Open in Google Maps
        </button>

        <div className="servicetrack-route-tools">
          <span><Route size={16} /> {routeSummary?.distance || "Distance pending"}</span>
          <span><Clock size={16} /> {routeSummary?.duration || "ETA pending"}</span>
          <button type="button" onClick={refreshProviderRoute} disabled={isClosed || locatingProvider || (!hasCoordinates(clientLocation) && !clientAddress.trim())}>
            <LocateFixed size={16} />
            {locatingProvider ? "Locating" : "Refresh route"}
          </button>
        </div>

        <div className="servicetrack-live-line" data-live={connected && liveSharing ? "true" : "false"}>
          <span />
          {connected && liveSharing ? "Live location updates every 5 seconds" : connected ? "Socket ready for job status" : "Connecting live sync"}
        </div>
        ...
        <div>
          ...
          <div className="servicetrack-status-grid">
            ...
            <button type="button" onClick={markJobStarted} disabled={!canStartJob || normalizedStatus !== "arrived"} data-active={normalizedStatus === "job_started"}>
              <Route size={17} />
              Job started
            </button>
            <button type="button" onClick={markCompleted} disabled={!canFinishJob || normalizedStatus !== "job_started"} data-active={normalizedStatus === "completed"}>
