...
export default function Home() {
 ...
 const refreshCatalogProviders = useCallback(async () => {
  fetch(`${API_URL}/catalog`)
    .then((response) =>
      response.ok ? response.json() : { providers: [] }
    )
    .then((data) =>
      setCatalogProviders(
 ...
 const filteredServices = marketplaceServices
  .filter((service) => {
    const haystack = [
      service.name,
      service.category,
 ...
 .sort((a, b) => {
  ...
  const trustedA = reviewsA >= 3;
  const trustedB = reviewsB >= 3;

  // 1. Minimum 3 reviews first
