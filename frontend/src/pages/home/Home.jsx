...
export default function Home() {
  ...
  const updateProviderBookingStatus = async (bookingId, status, cancellationReason = "") => {
    try {
      ...
      setStatusMessage(`Booking marked ${status}.`);
      return data.booking;
    } catch (error) {
      setStatusMessage(error.message);
      return false;
  ...
  return (
    <div className={isDark ? "dark" : ""}>
      <div className="min-h-screen bg-[linear-gradient(135deg,#f8fafc_0%,#ecfeff_38%,#fff7ed_100%)] text-slate-950 transition-colors duration-500 dark:bg-[linear-gradient(135deg,#020617_0%,#07111f_48%,#111827_100%)] dark:text-white">
            ...
            <div className="hidden flex-none items-center gap-3 xl:flex">
              ...
              {user ? (
                <>
                  ...
                  <ProfileAvatar user={user} onClick={openProfileMenu} size="sm" />
                  <button type="button" onClick={handleLogout} className="whitespace-nowrap rounded-full bg-slate-950 px-5 py-3 text-sm font-black text-white shadow-xl shadow-slate-950/15 [overflow-wrap:normal] transition hover:-translate-y-0.5 dark:bg-white dark:text-slate-950">{t("logout")}</button>
                </>
              ) : (
                <>
                  <button type="button" onClick={() => openClientAuth("login")} className="whitespace-nowrap rounded-full px-5 py-3 text-sm font-black text-slate-700 [overflow-wrap:normal] transition hover:bg-slate-100 dark:text-white dark:hover:bg-white/10">{t("login")}</button>
                  <button type="button" onClick={() => openProviderAuth("register")} className="whitespace-nowrap rounded-full bg-gradient-to-r from-teal-600 to-blue-600 px-5 py-3 text-sm font-black text-white shadow-xl shadow-blue-600/20 [overflow-wrap:normal] transition hover:-translate-y-0.5">{t("becomeProvider")}</button>
                  <LanguageSwitcher language={language} setLanguage={setLanguage} t={t} />
                </>
              )}
            </div>
        ...
        <button type="button" onClick={() => setChatOpen((current) => !current)} className="fixed bottom-5 right-5 z-40 grid h-14 w-14 place-items-center rounded-2xl bg-amber-300 text-slate-950 shadow-2xl shadow-amber-300/40">
      ...
      {authMode && (
        <AuthModal
          ...
          onAuthSuccess={(nextUser) => {
            setUser(nextUser);
            setActiveView(nextUser.role === "provider" ? "provider" : "home");
...
function ClientDashboard({ bookings, setActiveView, cancelClientBooking, onAcceptEstimate, onRejectEstimate, onSubmitReview, onPayNow, payingBookingId, t = (key) => key, isProviderClientMode = false, onBrowseServices, onProviderDashboard }) {
  ...
  const bookingSections = [
    {
      ...
      bookings: bookings.filter((booking) => ["pending", "accepted"].includes(booking.status)),
    },
    {
      title: "Active bookings",
      copy: "Confirmed and in-progress services appear here.",
      bookings: bookings.filter((booking) => ["confirmed", "assigned", "on_the_way", "en_route", "arrived", "job_started"].includes(booking.status)),
    },
    {
  ...
  const statusBlocks = [
    ...
    { title: "Active", value: bookingSections[1].bookings.length, copy: "Confirmed and live work.", tone: "from-blue-50 to-white text-blue-700" },
  ...
  return (
    <DashboardShell title={isProviderClientMode ? t("clientBookingDashboard") : t("clientDashboardTitle")} subtitle={isProviderClientMode ? t("providerClientSubtitle") : t("clientDashboardSubtitle")} notifications={notifications} workspaceLabel={t("workspace")}>
      ...
      <div className="grid gap-5 lg:grid-cols-3">
        ...
        <StatCard icon={Heart} label="Saved providers" value="8" />
          ...
          <div className="grid gap-4">
            {confirmedJobs.length ? confirmedJobs.map((booking) => (
              <div key={booking._id} className="grid gap-3">
                <JobCard
                  booking={booking}
                  actionLabel={booking.paymentStatus === "paid" ? "Mark completed" : "Waiting for payment"}
                  disabled={booking.status === "completed" || booking.paymentStatus !== "paid"}
                  onAction={() => updateProviderBookingStatus(booking._id, "completed")}
                  secondaryAction={() => setCancelTargetBooking(booking)}
                  onEstimateClick={() => setEstimateTargetBooking(booking)}
                />
              </div>
...
function AdminPanel({ adminData, selectedProviders, setSelectedProviders, updateProviderApproval, updateBookingRequest, setAdminData, refreshAdminContactMessages, setStatusMessage, adminEmail, paymentData, refreshAdminPayments }) {
  ...
  useEffect(() => {
    if (clientRequestsOpen || completedHistoryOpen || clientMessagesOpen) {
      window.scrollTo({ top: 0, behavior: "smooth" });
    }
  }, [clientRequestsOpen, completedHistoryOpen, clientMessagesOpen]);
        ...
        <div className="mt-4 rounded-2xl border border-teal-100 bg-teal-50 p-4 dark:border-teal-400/20 dark:bg-teal-400/10">
          ...
          {onEstimateClick && (
            <button type="button" onClick={onEstimateClick} className="mt-3 inline-flex items-center gap-2 rounded-xl bg-white px-4 py-3 text-sm font-black text-teal-800 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md dark:bg-white/10 dark:text-teal-100">
              <IndianRupee size={17} /> Submit Final Estimate
            </button>
          )}
        </div>
      )}
...
function ChatBox({ user, onClose, onServiceClick, onDashboardClick, onProviderSignup, onProviderDashboard, onAdminDashboard, onLogin, onContact }) {
  ...
  const [messages, setMessages] = useState([
    {
      ...
      role: "bot",
      text: user?.name
        ? `Hi ${user.name.split(" ")[0]}, I'm Mira from ServiceHub support. Tell me what is stuck and I will point you to the right place.`
        : "Hi, I'm Mira from ServiceHub support. I can help with booking status, payments, provider signup, or getting help from the team.",
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 18, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 18, scale: 0.96 }} className="fixed bottom-24 right-5 z-40 w-[min(420px,calc(100vw-32px))] overflow-hidden rounded-[1.7rem] border border-slate-200 bg-white shadow-2xl shadow-slate-950/20 dark:border-white/10 dark:bg-slate-900">
      <div className="flex items-center justify-between border-b border-slate-100 bg-gradient-to-r from-slate-950 to-teal-800 px-4 py-4 text-white dark:border-white/10">
        <div className="flex min-w-0 items-center gap-3">
          <div className="grid h-11 w-11 place-items-center rounded-2xl bg-white/12 ring-1 ring-white/20">
            <MessageCircle size={20} />
          </div>
          <div className="min-w-0">
            <p className="font-black">Mira, ServiceHub support</p>
        ...
        <button type="button" onClick={onClose} className="grid h-9 w-9 place-items-center rounded-full bg-white/10 text-white transition hover:bg-white/20" aria-label="Close chatbot">
          <X size={17} />
        </button>
      </div>
      <div className="max-h-[24rem] overflow-y-auto bg-slate-50 px-4 py-4 dark:bg-slate-950/40">
          ...
          {messages.map((message, index) => (
            <div key={`${message.role}-${index}`} className={`grid gap-2 ${message.role === "user" ? "justify-items-end" : "justify-items-start"}`}>
              <div className={`max-w-[88%] ${message.role === "user" ? "text-right" : "text-left"}`}>
                <p className={`rounded-2xl px-4 py-3 font-semibold leading-6 shadow-sm ${message.role === "user" ? "rounded-br-md bg-amber-300 text-slate-950" : "rounded-bl-md bg-white text-slate-800 ring-1 ring-slate-200 dark:bg-white/10 dark:text-white dark:ring-white/10"}`}>
                  {message.text}
                </p>
      ...
      <div className="border-t border-slate-100 bg-white p-4 dark:border-white/10 dark:bg-slate-900">
        ...
        >
          ...
          <button type="submit" disabled={isTyping || !input.trim()} className="grid h-10 w-10 place-items-center rounded-xl bg-slate-950 text-white transition hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-45 dark:bg-amber-300 dark:text-slate-950" aria-label="Send message"><Send size={17} /></button>
        </form>
      </div>
    </motion.div>
  );
